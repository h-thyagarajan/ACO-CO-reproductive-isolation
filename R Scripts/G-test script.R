...
Example.Mat = t(as.matrix(table(dat1$female.id, dat1$mated.male.pop, dat1$trial.id)[1,,]))

## step 1:  Individual G-value
# repeat this for each trial (1-4)
#example for A1 female trial 1

observed =Example.Mat[1,]
expected =c(0.5,0.5)
...
## step 2: G-test of independence

GTest(Example.Mat, correct="none") 
#output gives heterogeneity G value, df, p-value (testing for significant heterogeneity between trials)

## step 3: Pooled G-value- use values pooled across replicates and expected proportions under null hypothesis
